...
const ApprovalsDashboard = () => {
  ...
  const loadData = async () => {
    try {
      ...
      const pending = visitorsData.filter(v => v.status === 'pre_registered' || v.status === 'pending_approval').length
  ...
  const filteredVisitors = visitors.filter(visitor => {
    ...
    if (filter === 'pending') {
      return visitor.status === 'pre_registered' || visitor.status === 'pending_approval'
    }
    if (filter === 'approved') {
      ...
      <div className="space-y-6">
        ...
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
          <div>
            ...
            <p className="text-gray-500">Manage your visitor pre-registrations and approvals</p>
          </div>
          <button
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
          <Card className="rounded-2xl shadow-sm border-none ring-1 ring-gray-100">
            <Flex alignItems="start" justifyContent="between">
              <div>
                ...
                <Metric className="mt-2 text-3xl font-bold text-gray-900">{stats.pending}</Metric>
              </div>
          ...
          <Card className="rounded-2xl shadow-sm border-none ring-1 ring-gray-100">
            <Flex alignItems="start" justifyContent="between">
              <div>
                <Text className="text-gray-500 font-medium">Approved</Text>
                <Metric className="mt-2 text-3xl font-bold text-gray-900">{stats.approved}</Metric>
              </div>
          ...
          <Card className="rounded-2xl shadow-sm border-none ring-1 ring-gray-100">
            <Flex alignItems="start" justifyContent="between">
              <div>
                <Text className="text-gray-500 font-medium">Today's Visits</Text>
                <Metric className="mt-2 text-3xl font-bold text-gray-900">{stats.todayVisits}</Metric>
              </div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                {filteredVisitors.map((visitor, index) => (
                  <motion.div
                    ...
                    className="group bg-white p-5 border border-gray-200 rounded-xl hover:shadow-md hover:border-blue-200 transition-all duration-200"
                  >
                    <div className="flex items-start justify-between mb-4">
                      <div className="flex items-center gap-3">
                        <div className="w-10 h-10 rounded-full bg-gray-100 flex items-center justify-center text-gray-600 font-bold text-sm group-hover:bg-blue-50 group-hover:text-blue-700 transition-colors">
                          {visitor.name?.charAt(0) || 'V'}
                        </div>
                        <div>
                          <h3 className="font-semibold text-gray-900 leading-tight">{visitor.name}</h3>
                          <p className="text-xs text-gray-500 mt-0.5">{visitor.company || 'No Company'}</p>
                        </div>
                      </div>
                    ...
                    {visitor.status === 'pending_approval' && (
                      <div className="flex gap-2 pt-2 border-t border-gray-50">
                        <button
                          ...
                          className="flex-1 px-3 py-2 bg-slate-900 text-white rounded-lg hover:bg-slate-800 text-sm font-medium transition-colors flex items-center justify-center gap-1"
                        >
                          <CheckCircle size={14} /> Approve
                        </button>
                        <button
                          onClick={() => handleReject(visitor.id)}
                          className="flex-1 px-3 py-2 bg-red-50 text-red-700 rounded-lg hover:bg-red-100 text-sm font-medium transition-colors flex items-center justify-center gap-1"
                        >
          ...
          >
            <div className="p-6 md:p-8">
              <div className="flex items-center justify-between mb-8">
                <div>
                  <h2 className="text-2xl font-bold text-gray-900">Pre-register Visitor</h2>
                  <p className="text-gray-500 text-sm mt-1">Fill in the details to schedule a visit</p>
                </div>
                <button
              ...
              <form onSubmit={handlePreRegister} className="space-y-6">
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                  <div className="space-y-1">
                    ...
                    <input
                      ...
                      className="w-full px-4 py-2.5 border border-gray-300 rounded-xl focus:ring-2 focus:ring-slate-900 focus:border-transparent transition-all outline-none"
                ...
                <div className="space-y-1">
                  ...
                  <textarea
                    ...
                    className="w-full px-4 py-2.5 border border-gray-300 rounded-xl focus:ring-2 focus:ring-slate-900 focus:border-transparent transition-all outline-none resize-none"
                ...
                <div className="flex gap-4 pt-4 border-t border-gray-100 mt-6">
                  ...
                  <button
                    type="submit"
                    className="flex-1 px-4 py-3 bg-slate-900 text-white rounded-xl hover:bg-slate-800 font-medium shadow-lg shadow-slate-900/20 transition-all transform active:scale-[0.98]"
                  >
                    Pre-register Visitor
                  </button>
                </div>
